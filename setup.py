...
def setup_swap(device, size_gb, stdscr):
    ...
    stdscr.refresh()
    try:
        subprocess.run(f"sudo dd if=/dev/zero of={device} bs=1G count={size_gb}", shell=True, check=True)
        subprocess.run(f"sudo mkswap {device}", shell=True, check=True)
        subprocess.run(f"sudo swapon {device}", shell=True, check=True)
        stdscr.addstr(3,2, "Swap setup complete!")
    except Exception as e:
        stdscr.addstr(3,2, f"Error setting up swap: {e}")
    stdscr.refresh()
    stdscr.getch()
...
def install_packages(pkgs, stdscr):
    ...
    stdscr.refresh()
    try:
        subprocess.run(f"sudo dnf install -y {' '.join(pkgs)}", shell=True, check=True)
        stdscr.addstr(3,2, "Installation complete!")
    except Exception as e:
        stdscr.addstr(3,2, f"Error during install: {e}")
    stdscr.refresh()
    stdscr.getch()
...
def main(stdscr):
    ...
    stdscr.clear()
    stdscr.addstr(2,2, "Fedora setup complete! Press any key to exit.")
    stdscr.refresh()
    stdscr.getch()
